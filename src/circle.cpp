...
namespace simplecpp {
Circle::Circle(Composite *owner) : Sprite(0, 0, owner) {
    radius = 0;
    show();
}
...
void Circle::init(const double x, const double y, const double r,
                  bool repaintP) {
    reset(x, y, r, repaintP);
}

void Circle::reset(const double x, const double y, const double r,
                   bool repaintP) {
    pose = Pose(x, y);
    radius = r;
    if (repaintP)
