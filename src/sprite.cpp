...
void Sprite::move(double xshift, double yshift, bool repaintP) {
    ...
    if (penIsDown) {
        Line l(oldorigin.x, oldorigin.y, oldorigin.x + xshift,
               oldorigin.y + yshift);
...
void Sprite::penDown(bool down) { penIsDown = down; }

void Sprite::penUp(bool up) { penIsDown = !up; }
...
float Sprite::getZIndex() const { return z_index; }

Position Sprite::getAtan2() const { return pose.getAtan2(); }
} // namespace simplecpp
