...
class Pose {
  ...
  public:
    ...
    Position prodPosition(Position p) const {
        return Position(a11 * p.x + a12 * p.y + a13,
                        a21 * p.x + a22 * p.y + a23);
    }

    void compute_res_vertex(vector<XPoint> &xpt, const vector<Position> &vertex,
                            int count) const {
    ...
    Position getOrigin() const { return Position(a13, a23); }

    Position getAtan2() const { return Position(a11, a21); }

    double getOrientation() const { return atan2(a21, a11); }
